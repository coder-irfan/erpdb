...
const DEFAULT_DICTIONARY = {
  title: 'Notifications', newCount: '{count} new', markAllRead: 'Mark all as read', clearAll: 'Clear all',
  emptyTitle: 'No new notifications', emptyDescription: 'Recent activity relevant to your role will appear here.',
  viewAll: 'View All Notifications', performedBy: 'Performed by {name}', systemActor: 'System', entities: {}, events: {}
}
...
const getEntityKey = action => [
  'FINANCE_EXPENSE', 'FINANCE_INCOME', 'FINANCE_SALARY', 'FINANCE_LOAN', 'INVENTORY_ITEM', 'HRM_CONTRACT',
  'HRM_STAFF', 'CRM_VISITOR', 'CRM_CLIENT', 'CRM_LEAD', 'CONTRACT', 'INVOICE', 'PROJECT', 'TASK', 'LEAVE',
  'OPTION', 'USER', 'ROLE'
].find(key => action === key || action.startsWith(`${key}_`)) || 'SYSTEM'

const getEventKey = action => [
  'REPAYMENT_RECORDED', 'PAYMENT_RECORDED', 'QUANTITY_ADJUSTED', 'MEMBER_ASSIGNED', 'MEMBER_REMOVED',
  'ACTIVITY_CREATED', 'ACTIVITY_ADDED', 'STATUS_UPDATED', 'MARKED_PAID', 'HOURS_LOGGED', 'CHECKED_OUT',
  'CHECKED_IN', 'CREATED', 'UPDATED', 'DELETED', 'CONVERTED', 'PAID'
...
const NotificationDropdown = ({ dictionary: providedDictionary }) => {
  ...
  const persistState = useCallback((nextRead, nextDismissed, owner = userId) => {
    if (!owner || owner === 'anonymous') return
    localStorage.setItem(`audit-notifications:${owner}`, JSON.stringify({ read: [...nextRead], dismissed: [...nextDismissed] }))
  }, [userId])
  ...
  return (
    <>
      <IconButton ref={anchorRef} onClick={() => { setOpen(value => !value); void loadFeed() }} className='text-textPrimary' aria-label={dictionary.title}>
        <Badge
          color='error'
          ...
          overlap='circular'
          sx={{ '& .MuiBadge-badge': { minWidth: 16, height: 16, padding: '0 3px', fontSize: '0.625rem', lineHeight: 1, top: 3, right: 2 } }}
        >
          <i className='tabler-bell' />
        </Badge>
      </IconButton>
      <Popper open={open} transition disablePortal placement='bottom-end' ref={popperRef} anchorEl={anchorRef.current} {...(isSmallScreen ? { className: 'is-[calc(100vw-2rem)] max-is-[380px] !mbs-3 z-[1] max-bs-[550px] bs-[550px]', modifiers: [{ name: 'preventOverflow', options: { padding: themeConfig.layoutPadding } }] } : { className: 'is-[380px] !mbs-3 z-[1] max-bs-[550px] bs-[550px]' })}>
              ...
              <ClickAwayListener onClickAway={() => setOpen(false)}>
                <div className='bs-full flex flex-col'>
                  <div className='flex items-center gap-2 pli-4 plb-3.5'>
                    ...
                    <Tooltip title={dictionary.markAllRead}><span><IconButton size='small' disabled={unreadCount === 0} onClick={markAllRead}><i className='tabler-mail-opened' /></IconButton></span></Tooltip>
                    <Tooltip title={dictionary.clearAll}><span><IconButton size='small' disabled={visibleFeed.length === 0} onClick={clearAll}><i className='tabler-trash' /></IconButton></span></Tooltip>
                  </div>
                  <Divider />
                  <ScrollWrapper hidden={hidden}>
                    {visibleFeed.length === 0 ? (
                      ...
                      return (
                        <div key={notification.id} className={classnames('group flex cursor-pointer gap-3 pli-4 plb-3 hover:bg-actionHover', { 'border-be': index !== visibleFeed.length - 1, 'bg-actionHover/50': unread })} onClick={() => markRead(notification.id)}>
                          <CustomAvatar color={notification.avatarColor} skin='light-static'><i className={notification.avatarIcon} /></CustomAvatar>
                          <div className='min-is-0 flex-auto'>
                            <Typography variant='body2' className='font-medium' color='text.primary'>{notificationTitle(notification, dictionary)}</Typography>
                            <Typography variant='caption' color='text.secondary' className='mt-1 block'>{notificationSubtitle(notification, dictionary)}</Typography>
                            <Typography variant='caption' color='text.disabled'>{relativeTime(notification.timestamp, locale)}</Typography>
                          </div>
                  ...
                  <Divider />
                  <div className='p-4'><Button component={Link} href={getLocalizedUrl('/options/audit-logs', locale)} onClick={() => setOpen(false)} fullWidth variant='contained' size='small'>{dictionary.viewAll}</Button></div>
                </div>
              </ClickAwayListener>
            </Paper>
          </Fade>
