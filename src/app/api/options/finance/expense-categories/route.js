...
export async function GET() {
  ...
  if (!authorization.authorized) {
    return Response.json(
      { success: false, code: authorization.code, error: 'Expense categories could not be loaded.' },
  ...
  try {
    ...
    return Response.json(
      { success: false, code: 'EXPENSE_CATEGORIES_LOAD_FAILED', error: 'Expense categories could not be loaded.' },
      { status: 500 }
    )
  }
}
