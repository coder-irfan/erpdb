...
const ContractPoliciesPage = async props => {
  const { lang } = await props.params
  const dictionary = await getDictionary(lang)
  const session = await getServerSession(authOptions)
  const result = await getOptionsListPaginated({ category: 'CONTRACT_POLICY', locale: lang })
...
export default ContractPoliciesPage
