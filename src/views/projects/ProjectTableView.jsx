...
const ProjectTableView = ({ data, loading, page, rowsPerPage, locale, dictionary, canWrite, canDelete, onPageChange, onRowsPerPageChange, onView, onEdit, onMembers, onDelete, onAdd }) => (
  <>
        ...
        <tbody>
          {loading ? <TableSkeletonRows columns={8} /> : data.projects.length === 0 ? (
            ...
            <tr key={project.id} onClick={() => onView(project)}>
              ...
              <td className='text-end' onClick={event => event.stopPropagation()}><EntityActionsMenu moreActionsLabel={dictionary.table.actions} actions={[{ label: dictionary.actions.view, icon: 'tabler-eye', onClick: () => onView(project) }, canWrite && { label: dictionary.actions.edit, icon: 'tabler-edit', onClick: () => onEdit(project) }, canWrite && { label: dictionary.actions.manageMembers, icon: 'tabler-users-plus', onClick: () => onMembers(project) }, canDelete && { label: dictionary.actions.delete, icon: 'tabler-trash', color: 'error', onClick: () => onDelete(project) }]} /></td>
            </tr>
          ))}
        </tbody>
      </table>
