...
const emptyValues = options => ({
  title: '', description: '', client_id: '', contract_id: '', project_manager_id: '',
  status_id: defaultOption(options.statuses), priority_id: defaultOption(options.priorities),
  project_area: '', project_sponsor: '', estimated_hours: '0', actual_hours: '0', budget: '',
  currency: options.baseCurrency || 'AFN', exchange_rate: String(options.exchangeRate || '65'),
  start_date: toDateInputValue(new Date()), end_date: toDateInputValue(new Date()), actual_end_date: ''
})

const ProjectFormDrawer = ({ open, project, options, locale, dictionary, onClose, onSaved }) => {
  const { control, handleSubmit, reset, formState: { errors, isSubmitting } } = useForm({ resolver: valibotResolver(createProjectSchema(dictionary.validation)), defaultValues: emptyValues(options) })
  const clientId = useWatch({ control, name: 'client_id' })
  const budget = useWatch({ control, name: 'budget' })
  const currency = useWatch({ control, name: 'currency' })
  const exchangeRate = useWatch({ control, name: 'exchange_rate' })
  const baseAmount = useMemo(() => convertToBaseCurrency(budget, currency, exchangeRate, options.baseCurrency), [budget, currency, exchangeRate, options.baseCurrency])
  const contracts = options.contracts.filter(contract => !clientId || contract.client_id === clientId)

  useEffect(() => {
    if (!open) return
    reset(project ? {
      title: project.title || '', description: project.description || '', client_id: project.client_id || '', contract_id: project.contract_id || '', project_manager_id: project.project_manager_id || '', status_id: project.status_id || defaultOption(options.statuses), priority_id: project.priority_id || defaultOption(options.priorities), project_area: project.project_area || '', project_sponsor: project.project_sponsor || '', estimated_hours: String(project.estimated_hours || '0'), actual_hours: String(project.actual_hours || '0'), budget: String(project.budget || ''), currency: project.currency || options.baseCurrency || 'AFN', exchange_rate: String(project.exchange_rate || options.exchangeRate || '65'), start_date: toDateInputValue(project.start_date), end_date: toDateInputValue(project.end_date), actual_end_date: toDateInputValue(project.actual_end_date)
    } : emptyValues(options))
  }, [open, options, project, reset])

  const submit = async values => {
    const result = project ? await updateProject(project.id, { ...values, locale }) : await createProject({ ...values, locale })
  ...
  const field = (name, label, props = {}) => <Controller name={name} control={control} render={({ field: input }) => <CustomTextField {...input} {...props} value={input.value ?? ''} label={label} error={Boolean(errors[name])} helperText={errors[name]?.message} />} />
  const select = (name, label, items, placeholder, allowEmpty = false) => <Controller name={name} control={control} render={({ field: input }) => <CustomTextField {...input} select value={input.value || ''} label={label} error={Boolean(errors[name])} helperText={errors[name]?.message} slotProps={{ select: { displayEmpty: true, renderValue: value => items.find(item => item.id === value)?.label || placeholder } }}>{allowEmpty && <MenuItem value=''>{placeholder}</MenuItem>}{items.map(item => <MenuItem key={item.id} value={item.id}>{item.label}</MenuItem>)}</CustomTextField>} />

  return (
    <Drawer anchor='right' open={open} onClose={isSubmitting ? undefined : onClose} PaperProps={{ className: 'is-full sm:is-[720px]' }}>
      ...
      <form className='form-surface-scroll flex flex-1 flex-col gap-5 p-5' onSubmit={handleSubmit(submit)} noValidate>
        <FormSectionCards labels={[dictionary.tabs?.general || 'Project information', dictionary.tabs?.planning || 'Planning and ownership', dictionary.tabs?.financial || 'Financial terms']}>
        ...
        <div className='grid grid-cols-1 gap-4 sm:grid-cols-2'>
          ...
          <Controller name='project_manager_id' control={control} render={({ field: input }) => <Autocomplete options={options.staff} value={options.staff.find(staff => staff.id === input.value) || null} onChange={(_, value) => input.onChange(value?.id || '')} getOptionLabel={option => option.full_name} isOptionEqualToValue={(option, value) => option.id === value.id} renderInput={params => <CustomTextField {...params} label={dictionary.fields.manager} placeholder={dictionary.placeholders.manager} error={Boolean(errors.project_manager_id)} helperText={errors.project_manager_id?.message} />} />} />
          ...
          {field('estimated_hours', dictionary.fields.estimatedHours, { type: 'number', inputProps: { min: 0, step: '0.25' } })}
