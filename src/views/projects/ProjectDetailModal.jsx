    ...
    <Dialog open={open} onClose={loading || working ? undefined : onClose} fullWidth maxWidth='lg'>
      ...
      <DialogContent dividers className='min-bs-[620px]'>
        {loading ? <div className='flex min-bs-[520px] items-center justify-center'><CircularProgress /></div> : error ? <Alert severity='error'>{error}</Alert> : project ? <div className='flex flex-col gap-5'>
          ...
          {tab === 3 && <div className='flex flex-col gap-4'><Card variant='outlined'><CardContent><div className='flex flex-wrap items-center justify-between gap-3'><div><Typography variant='h6'>{dictionary.detail.timesheets}</Typography><Typography color='text.secondary'>{timesheetHours} / {project.estimated_hours || 0}h</Typography></div><Chip variant='tonal' color='primary' label={`${Math.min(100, Math.round(timesheetHours / Math.max(toFiniteNumber(project.estimated_hours), 1) * 100))}%`} /></div><LinearProgress variant='determinate' value={Math.min(100, timesheetHours / Math.max(toFiniteNumber(project.estimated_hours), 1) * 100)} className='mt-4 bs-2 rounded' /></CardContent></Card>{project.timesheets.length === 0 ? <EmptyPanel icon='tabler-clock-off' text={dictionary.detail.noTimesheets} /> : <div className='no-scrollbar overflow-x-auto'><table className='w-full'><thead><tr className='border-be border-divider text-start'><th className='p-3'>{dictionary.detail.staff}</th><th className='p-3'>{dictionary.detail.date}</th><th className='p-3'>{dictionary.detail.hours}</th><th className='p-3'>{dictionary.detail.note}</th></tr></thead><tbody>{project.timesheets.map(row => <tr key={row.id} className='border-be border-divider'><td className='p-3'><Typography variant='body2'>{row.staff.full_name}</Typography></td><td className='p-3'>{toDateInputValue(row.date)}</td><td className='p-3'>{row.hours_worked || 0}h</td><td className='max-is-[280px] p-3'><Typography variant='body2' className='truncate'>{row.notes || '—'}</Typography></td></tr>)}</tbody></table></div>}</div>}
          {tab === 4 && <div className='flex flex-col gap-4'><div className='grid grid-cols-1 gap-4 sm:grid-cols-3'>{financeCards.map(([label, value, colorClass]) => <Card key={label} variant='outlined'><CardContent><Typography color='text.secondary'>{label}</Typography><Typography variant='h5' className={`mt-1 ${colorClass}`}>{formatCurrency(value, locale, baseCurrency)}</Typography></CardContent></Card>)}</div>{project.expenses.length + project.incomes.length === 0 ? <EmptyPanel icon='tabler-cash-off' text={dictionary.detail.noFinances} /> : <div className='no-scrollbar overflow-x-auto'><table className='w-full'><thead><tr className='border-be border-divider'><th className='p-3 text-start'>{dictionary.detail.type}</th><th className='p-3 text-start'>{dictionary.detail.note}</th><th className='p-3 text-start'>{dictionary.detail.date}</th><th className='p-3 text-end'>{dictionary.detail.amount}</th></tr></thead><tbody>{project.incomes.map(row => <tr key={`i-${row.id}`} className='border-be border-divider'><td className='p-3'><Chip size='small' variant='tonal' color='success' label={dictionary.detail.revenue} /></td><td className='p-3'><Typography variant='body2' className='max-is-[280px] truncate'>{row.name}</Typography></td><td className='p-3'>{toDateInputValue(row.created_at)}</td><td className='p-3 text-end text-success'>{formatCurrency(row.total_amount, locale, row.currency)}</td></tr>)}{project.expenses.map(row => <tr key={`e-${row.id}`} className='border-be border-divider'><td className='p-3'><Chip size='small' variant='tonal' color='error' label={dictionary.detail.expenses} /></td><td className='p-3'><Typography variant='body2' className='max-is-[280px] truncate'>{row.details}</Typography></td><td className='p-3'>{toDateInputValue(row.expense_date)}</td><td className='p-3 text-end text-error'>{formatCurrency(row.sub_total, locale, row.currency)}</td></tr>)}</tbody></table></div>}</div>}
        </div> : null}
      </DialogContent>
    </Dialog>
  )
