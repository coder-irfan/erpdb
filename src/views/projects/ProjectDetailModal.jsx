...
const statusChipProps = option => {
  ...
  if (/^#[0-9a-f]{6}$/i.test(configuredColor || ''))
    return { sx: { color: configuredColor, backgroundColor: `${configuredColor}18` } }
...
const Item = ({ label, value, accent }) => (
  <div>
    ...
    <Typography className={`mt-1 break-words ${accent || ''}`}>{value || '—'}</Typography>
  </div>
)

const EmptyPanel = ({ icon, text }) => (
  <div className='flex min-bs-[240px] flex-col items-center justify-center text-center'>
    <i className={`${icon} mb-3 text-4xl text-textDisabled`} />
      ...
      <table className='w-full'>
        <thead>
          <tr className='text-start'>
            <th className='p-3'>{dictionary.detail.staff}</th>
        ...
        <tbody>
          {rows.map(row => (
            <tr key={row.id}>
              <td className='p-3'>
                <Typography variant='body2'>{row.staff.full_name}</Typography>
              </td>
              <td className='p-3'>{toDateInputValue(row.date)}</td>
              <td className='p-3'>{row.hours_worked || 0}h</td>
              <td className='max-is-[280px] p-3'>
...
const ProjectFinanceTable = ({ project, dictionary, locale }) => {
  ...
  const typeChip = row => (
    <Chip
      size='small'
      variant='tonal'
      color={row.rowType === 'income' ? 'success' : 'error'}
      label={row.rowType === 'income' ? dictionary.detail.revenue : dictionary.detail.expenses}
    />
  )
    ...
    <ResponsiveDataTable
      ...
      mobileMetadata={[
        ...
        { id: 'date', label: dictionary.detail.date, render: row => toDateInputValue(row.date) },
        {
          ...
          render: row => formatCurrency(row.amount, locale, row.currency)
        }
      ]}
    >
      <div className='no-scrollbar overflow-x-auto'>
        <table className='w-full'>
          <thead>
          <tr>
              <th className='p-3 text-start'>{dictionary.detail.type}</th>
              <th className='p-3 text-start'>{dictionary.detail.note}</th>
              <th className='p-3 text-start'>{dictionary.detail.date}</th>
          ...
          <tbody>
            {rows.map(row => (
              <tr key={`${row.rowType}-${row.id}`}>
                <td className='p-3'>{typeChip(row)}</td>
                <td className='p-3'>
                  <Typography variant='body2' className='max-is-[280px] truncate'>
                    {row.note || '—'}
                  </Typography>
                </td>
                <td className='p-3'>{toDateInputValue(row.date)}</td>
...
const ProjectDetailModal = ({
  ...
  const load = async () => {
    ...
    if (result.success) {
      ...
      setContractId(result.data.contract_id || '')
  ...
  const eligibleStaff = useMemo(
    () => options.staff.filter(staff => !project?.members.some(member => member.staff.id === staff.id)),
    [options.staff, project?.members]
  )

  const eligibleContracts = useMemo(
    () => options.contracts.filter(contract => contract.client_id === project?.client_id),
    [options.contracts, project?.client_id]
  )
        ...
        <div className='min-is-0'>
          <div className='flex flex-wrap items-center gap-2'>
            <Typography variant='h5' className='truncate'>
              {project?.title || dictionary.actions.view}
            </Typography>
            {project && (
              <Chip size='small' variant='tonal' label={project.status.label} {...statusChipProps(project.status)} />
            )}
          </div>
          <Typography color='text.secondary'>
            {project ? `${project.project_code} · ${project.client.company_name}` : dictionary.common.loading}
          </Typography>
        </div>
          ...
          <div className='flex flex-col gap-5'>
            <Tabs value={tab} onChange={(_, value) => setTab(value)} variant='scrollable' scrollButtons='auto'>
              ...
              <Tab
                ...
                iconPosition='start'
                label={dictionary.detail.contract}
              />
              <Tab icon={<i className='tabler-clock' />} iconPosition='start' label={dictionary.detail.timesheets} />
              <Tab icon={<i className='tabler-chart-pie' />} iconPosition='start' label={dictionary.detail.finances} />
            </Tabs>
            {tab === 0 && (
                ...
                <Card variant='outlined'>
                  <CardContent>
                    ...
                    <div className='grid grid-cols-2 gap-4'>
                      ...
                      <Item label={dictionary.fields.manager} value={project.project_manager?.full_name} />
                      <Item label={dictionary.fields.client} value={project.client.company_name} />
                    </div>
                  </CardContent>
                </Card>
                <Card variant='outlined'>
                  <CardContent>
                    <Typography variant='h6' className='mb-4'>
                      {dictionary.detail.timeline}
                    </Typography>
                    <div className='grid grid-cols-2 gap-4'>
                      <Item label={dictionary.fields.startDate} value={toDateInputValue(project.start_date)} />
                      <Item label={dictionary.fields.endDate} value={toDateInputValue(project.end_date)} />
                ...
                <Card variant='outlined' className='lg:col-span-2'>
                  <CardContent>
                    ...
                    <div className='grid grid-cols-2 gap-4 md:grid-cols-4'>
                      ...
                      />
                      <Item
                        label={dictionary.detail.baseAmount}
                        value={formatCurrency(project.amount_base, locale, baseCurrency)}
                        accent='font-semibold text-success'
                      />
                      <Item label={dictionary.fields.currency} value={project.currency} />
                      <Item label={dictionary.detail.rate} value={project.exchange_rate} />
                    </div>
                  </CardContent>
                </Card>
              </div>
            ...
            {tab === 1 && (
              <div className='flex flex-col gap-4'>
                {canWrite && (
                  <Card variant='outlined'>
                    <CardContent className='grid grid-cols-1 items-end gap-3 md:grid-cols-[1fr_1fr_auto]'>
                      <CustomTextField
                        select
                        label={dictionary.fields.member}
                        value={staffId}
                ...
                {project.members.length === 0 ? (
                  <EmptyPanel icon='tabler-users-minus' text={dictionary.detail.noMembers} />
                ) : (
                  <div className='grid grid-cols-1 gap-3 md:grid-cols-2'>
                    {project.members.map(member => (
                      <Card key={member.id} variant='outlined'>
                        <CardContent className='flex items-center gap-3'>
                          <UserAvatar user={member.staff} size={40} />
                          <div className='min-is-0 grow'>
                            <Typography className='truncate font-medium'>{member.staff.full_name}</Typography>
                            <Typography variant='body2' color='text.secondary' className='truncate'>
                              {member.role || member.staff.position}
                            </Typography>
                            <Typography variant='caption' color='text.secondary'>
                              {dictionary.detail.assigned}: {toDateInputValue(member.assigned_at)}
                            </Typography>
                          </div>
                          {canWrite && (
                            <IconButton
                              color='error'
                              disabled={working}
                              aria-label={dictionary.actions.remove}
                  ...
                  <Card variant='outlined'>
                    <CardContent className='flex flex-col items-end gap-3 sm:flex-row'>
                      ...
                      <Button
                        ...
                        variant='contained'
                        disabled={working || contractId === (project.contract_id || '')}
                        onClick={() => updateContract(contractId)}
                      >
                        {contractId ? dictionary.actions.link : dictionary.actions.unlink}
                      </Button>
                    </CardContent>
                  </Card>
                )}
                {!project.contract ? (
                  <EmptyPanel icon='tabler-file-off' text={dictionary.detail.noContract} />
                ) : (
                  <Card variant='outlined'>
                    <CardContent>
                      <div className='mb-4 flex items-center gap-3'>
                        ...
                        <div>
                          <Typography variant='h6'>{project.contract.title}</Typography>
                          <Typography color='text.secondary'>{project.contract.contract_number}</Typography>
                        </div>
                      </div>
                      <div className='grid grid-cols-2 gap-4 md:grid-cols-4'>
                        <Item label={dictionary.fields.status} value={project.contract.status.label} />
                        <Item
                          label={dictionary.fields.budget}
                          value={formatCurrency(project.contract.total_amount, locale, project.contract.currency)}
                        />
                        <Item
                          label={dictionary.fields.startDate}
                          value={toDateInputValue(project.contract.start_date)}
                        />
                        <Item label={dictionary.fields.endDate} value={toDateInputValue(project.contract.end_date)} />
                      </div>
                    </CardContent>
                  </Card>
                )}
                  ...
                  <CardContent>
                    <div className='flex flex-wrap items-center justify-between gap-3'>
                      ...
                      <Chip
                        variant='tonal'
                        color='primary'
                        label={`${Math.min(100, Math.round((timesheetHours / Math.max(toFiniteNumber(project.estimated_hours), 1)) * 100))}%`}
                      />
                    </div>
                    <LinearProgress
                      variant='determinate'
                      value={Math.min(
                        100,
                        (timesheetHours / Math.max(toFiniteNumber(project.estimated_hours), 1)) * 100
                      )}
                      className='mt-4 bs-2 rounded'
                    />
                  </CardContent>
                </Card>
                  ...
                  {financeCards.map(([label, value, colorClass]) => (
                    <Card key={label} variant='outlined'>
                      <CardContent>
                        ...
                        <Typography variant='h5' className={`mt-1 ${colorClass}`}>
                          {formatCurrency(value, locale, baseCurrency)}
                        </Typography>
                      </CardContent>
                    </Card>
                  ))}
                </div>
                {project.expenses.length + project.incomes.length === 0 ? (
                  <EmptyPanel icon='tabler-cash-off' text={dictionary.detail.noFinances} />
                ) : (
                  <ProjectFinanceTable project={project} dictionary={dictionary} locale={locale} />
                )}
              </div>
            )}
          </div>
