...
const dateTime = (value, locale) => value ? new Intl.DateTimeFormat(localeMap[locale] || 'en-US', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(value)) : '—'
...
const VisitorDetailDialog = ({ visitor, open, locale, dictionary, purposeLabel, onClose }) => {
  ...
  return (
    <DetailDialog open={open} onClose={onClose} title={visitor.full_name} subtitle={visitor.company_name || visitor.phone || visitor.email}>
      <div className='flex flex-col gap-5 sm:gap-6'>
        <div className='flex flex-wrap gap-2'>
          <Chip size='small' variant='tonal' color={visitor.status === 'CHECKED_IN' ? 'warning' : 'success'} label={dictionary.status[visitor.status] || visitor.status} />
        ...
        <section className='rounded-lg border border-divider/70 p-4'>
          ...
          <Typography color='text.secondary' className='mt-2 whitespace-pre-wrap text-sm'>{formatMetadata(visitor.notes) || '—'}</Typography>
        </section>
      </div>
    </DetailDialog>
  )
