    ...
    <DetailDialog open={open} onClose={onClose} title={lead.title} subtitle={lead.company_name || lead.contact_name}>
      <div className='flex flex-col gap-5 sm:gap-6'>
        ...
        <div className='grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3'>
          ...
          <Item label='Email' value={<QuickContact email={lead.email}>{lead.email}</QuickContact>} />
          ...
          {lead.activities?.length ? (
            <div className='flex flex-col gap-2'>
              {lead.activities.map(activity => (
                <div key={activity.id} className='rounded-lg border border-divider/70 p-3'>
                  <div className='flex flex-wrap items-center justify-between gap-2'>
                    <Typography className='text-sm font-medium'>{activity.title}</Typography>
                    <Chip size='small' variant='tonal' label={activity.activity_type} />
                  </div>
                  <Typography color='text.secondary' className='mt-1 whitespace-pre-wrap text-sm'>
                    {formatMetadata(activity.description) || '—'}
                  </Typography>
                  <Typography variant='caption' color='text.secondary' className='mt-2 block'>
                    {date(activity.activity_date, locale)}
                    {activity.staff?.full_name ? ` • ${activity.staff.full_name}` : ''}
                  </Typography>
                </div>
              ))}
            </div>
