...
const StaffContractStatsCards = ({ summary, locale, currency, dictionary }) => {
  const cards = [
    ...
    { key: 'expiringSoon', label: dictionary.expiringSoon, icon: 'tabler-calendar-time', classes: 'bg-secondaryLighter text-warning' },
    { key: 'draft', label: dictionary.draft, icon: 'tabler-file-pencil', classes: 'bg-primaryLighter text-primary' },
    { key: 'totalValue', label: dictionary.totalValue, icon: 'tabler-cash-banknote', classes: 'bg-infoLighter text-info', money: true }
  ]
        ...
        <Card key={card.key} className='min-w-[280px] snap-start xl:min-w-0'>
          <CardContent className='flex items-center justify-between gap-4'>
            <div className='min-is-0'>
              <Typography className='text-sm font-semibold md:text-base' color='text.secondary'>{card.label}</Typography>
