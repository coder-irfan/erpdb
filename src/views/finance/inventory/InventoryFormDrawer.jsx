...
const defaults = (options, item) => ({
  ...
  category_id: item?.category_id || options.categories.find(option => option.is_default)?.id || options.categories[0]?.id || '',
...
const InventoryFormDrawer = ({ open, item, options, locale, dictionary, onClose, onSaved }) => {
  const { control, handleSubmit, reset, formState: { errors, isSubmitting } } = useForm({ resolver: valibotResolver(inventoryItemSchema(dictionary.validation)), defaultValues: defaults(options, item) })
  ...
  const submit = async values => {
    const response = await fetch(item ? `/api/finance/inventory/${item.id}` : '/api/finance/inventory', { method: item ? 'PUT' : 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...values, locale }) })
