        ...
        {loading ? (
          ...
          <div className='flex flex-col gap-5'>
            <Card variant='outlined'>
              <CardContent>
                <Typography variant='h6' className='mb-4'>{print.employeeInformation}</Typography>
            ...
            <Card variant='outlined'>
              <CardContent>
                <Typography variant='h6' className='mb-4'>{print.attendanceRate}</Typography>
                <div className='grid grid-cols-2 gap-4 sm:grid-cols-4'>
                  ...
                  <InfoItem label={print.dailyRate} value={formatCurrency(salary.base_daily_rate, locale, salary.currency)} />
                </div>
              </CardContent>
            </Card>
            ...
            <div className='grid grid-cols-1 gap-4 md:grid-cols-2'>
              <Card variant='outlined'>
                <CardContent>
                  ...
                  <AmountRow label={dictionary.payslip.baseSalary} amount={salary.base_salary} salary={salary} locale={locale} />
                  <AmountRow label={dictionary.payslip.earnedPay} amount={salary.earned_salary} salary={salary} locale={locale} />
                  <AmountRow label={dictionary.payslip.bonus} amount={salary.bonus_amount} salary={salary} locale={locale} tone='text-success' />
                </CardContent>
              </Card>
              <Card variant='outlined'>
                <CardContent>
                  <Typography variant='h6'>{dictionary.payslip.deductions}</Typography>
                  <AmountRow label={dictionary.payslip.loan} amount={salary.loan_deduction} salary={salary} locale={locale} tone='text-error' />
                  <AmountRow label={dictionary.payslip.offDays} amount={salary.unpaid_leave_deduction} salary={salary} locale={locale} tone='text-error' />
                </CardContent>
              </Card>
            </div>
            ...
            <Card className='border border-primary/30 bg-primaryLighter'>
              <CardContent className='flex flex-wrap items-center justify-between gap-4'>
                <div>
                  <Typography variant='caption' color='text.secondary'>{dictionary.payslip.netPayable}</Typography>
                  <DualCurrencyAmount
            ...
            {salary.timesheet_summary && (
              <Card variant='outlined'>
                <CardContent>
                  <Typography variant='h6' className='mb-2'>{dictionary.fields.notes}</Typography>
                  <Typography color='text.secondary' className='whitespace-pre-wrap'>{salary.timesheet_summary}</Typography>
                </CardContent>
              </Card>
            )}
          </div>
