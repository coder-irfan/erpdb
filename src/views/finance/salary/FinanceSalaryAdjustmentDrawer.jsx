...
const FinanceSalaryAdjustmentDrawer = ({ open, salary, baseCurrency, locale, dictionary, onClose, onSaved }) => {
  ...
  useEffect(() => {
    if (open) reset(valuesFor(salary))
  }, [open, reset, salary])
    ...
    >
      <div className='form-surface-header flex items-start justify-between gap-4 border-be border-divider p-5'>
        <div>
          ...
          <Typography color='text.secondary'>{salary?.staff?.full_name}</Typography>
        </div>
      ...
      <form className='form-surface-scroll flex flex-1 flex-col gap-5 px-5' onSubmit={handleSubmit(submit)} noValidate>
        <FormSectionCards labels={[dictionary.tabs?.general || 'Salary adjustment']}>
          ...
          <div className='grid grid-cols-1 gap-4 sm:grid-cols-2'>
            {field('worked_days', dictionary.fields.workedDays, {
              type: 'number',
              inputProps: { min: 0, max: salary?.total_month_days, step: 0.5 }
            })}
            {field('off_days', dictionary.fields.offDays, {
              type: 'number',
              inputProps: { min: 0, max: salary?.total_month_days, step: 0.5 }
            })}
            {field('bonus_amount', dictionary.fields.bonus, { type: 'number', inputProps: { min: 0, step: '0.01' } })}
            {field('loan_deduction', dictionary.fields.loanDeduction, {
              type: 'number',
          ...
          <Card variant='outlined'>
            <CardContent>
              <Typography variant='h6' className='mb-4'>
                {dictionary.form.liveCalculation}
              </Typography>
              <div className='grid grid-cols-2 gap-4'>
                <div>
                  <Typography variant='caption' color='text.secondary'>
                    {dictionary.fields.dailyRate}
                  </Typography>
                  <Typography className='font-semibold'>
                    {formatCurrency(calculation.dailyRate, locale, currency)}
                  </Typography>
                </div>
                <div>
                  <Typography variant='caption' color='text.secondary'>
                    {dictionary.fields.earnedSalary}
                  </Typography>
                  <Typography className='font-semibold'>
                    {formatCurrency(calculation.earned, locale, currency)}
                  </Typography>
                </div>
                <div>
                  <Typography variant='caption' color='text.secondary'>
                    {dictionary.fields.payableAmount}
                  </Typography>
                  <Typography className='font-semibold text-primary'>
                    {formatCurrency(calculation.payable, locale, currency)}
                  </Typography>
                </div>
                <div>
                  <Typography variant='caption' color='text.secondary'>
                  ...
                  <Typography className='font-semibold'>
                    {formatCurrency(calculation.base, locale, baseCurrency)}
                  </Typography>
                </div>
              </div>
            </CardContent>
