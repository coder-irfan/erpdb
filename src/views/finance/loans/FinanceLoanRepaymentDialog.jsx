...
const FinanceLoanRepaymentDialog = ({ open, loan, locale, dictionary, onClose, onSaved }) => {
  ...
  useEffect(() => {
    if (open)
      setAmount(
        String(Math.min(toFiniteNumber(loan?.monthly_deduction), toFiniteNumber(loan?.remaining_balance)) || '')
      )
  }, [loan, open])
  ...
  const submit = async () => {
    ...
    const response = await fetch(`/api/finance/loans/${loan.id}/repay`, {
      ...
      body: JSON.stringify({ repayment_amount: amount, source: 'MANUAL', locale })
    })
    ...
    <Dialog open={open} onClose={saving ? undefined : onClose} fullWidth maxWidth='lg'>
      ...
      <DialogContent dividers className='flex flex-col gap-4'>
        <Typography color='text.secondary'>
          {dictionary.repayment.description.replace('{loan}', loan?.loan_number || '')}
        </Typography>
        <div className='rounded bg-errorLighter p-3'>
          <Typography variant='caption' color='text.secondary'>
            {dictionary.fields.remaining}
          </Typography>
