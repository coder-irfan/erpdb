import { boolean, email, maxLength, minValue, nonEmpty, number, object, optional, pipe, regex, string, trim } from 'valibot'
...
export const createLeadSchema = customMessages => {
  ...
  return object({
    ...
    contact_name: pipe(string(messages.required), trim(), nonEmpty(messages.required), maxLength(191, messages.tooLong)),
    ...
    next_follow_up_date: optional(pipe(string(messages.dateInvalid), trim(), regex(DATE_TIME_PATTERN, messages.dateInvalid)), ''),
